...
%.lp
\item[\ib{interrupt\_hp\_pw}]
Interrupt multi-turn actions when hit points or magical power is restored
(default on).
%.lp
\item[\ib{legacy}]
...
%.lp
\item[\ib{wear\_unified}]
`{\tt W}'ear and `{\tt T}'ake off work on rings, amulets and blindfolds the same
way they work on armor (default on).
%.lp
\item[\ib{windowtype}]
